...
from ride_values import RIDE_RATES  # your dynamic ride values
...
if final_source and final_dest:
    st.subheader("Available Rides (Calculated)")

    # Calculate distance & duration using ORS
    route_info = get_route(final_source, final_dest)
    distance = route_info.get("distance_km")
    duration = route_info.get("duration_min")

    if distance is not None and duration is not None:
        ride_data = []
        for app, rates in RIDE_RATES.items():
            price = round(rates['per_km'] * distance + rates['per_min'] * duration, 2)
            ride_data.append({"App": app, "Price (₹)": price, "ETA (min)": round(duration, 2)})
        ...
        st.warning("Could not calculate distance & duration. Check ORS API key and connectivity.")
